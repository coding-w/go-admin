...
const initOrderApi = initdb.InitOrderSystem + 1
...
func (i *initApi) TableCreated() bool {
	// 查数据库中是否存在特定的表
	return global.GA_DB.Migrator().HasTable(&system.SysApi{})
}

func (i *initApi) DataInserted() bool {
	if errors.Is(global.GA_DB.Where("path = ? AND method = ?", "/authorityBtn/canRemoveAuthorityBtn", "POST").
...
func (i *initApi) InitializeData(ctx context.Context) (context.Context, error) {
	...
	if err := global.GA_DB.Create(&entities).Error; err != nil {
		return ctx, errors.Wrap(err, system.SysApi{}.TableName()+"表数据初始化失败!")
	}
